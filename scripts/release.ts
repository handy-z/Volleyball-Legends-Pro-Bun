...
export async function runZipAndRelease(): Promise<void> {
  ...
  const zipResult =
    await $`powershell Compress-Archive -Path build/vbl-pro-bun.exe -DestinationPath build/vbl-pro-bun.zip -Force`
      .quiet()
      .nothrow();

  if (zipResult.exitCode === 0) {
    const exeSize =
      (await Bun.file("build/vbl-pro-bun.exe").size) / 1024 / 1024;
    const zipSize =
      (await Bun.file("build/vbl-pro-bun.zip").size) / 1024 / 1024;
    console.log(
